<template>
  <el-card>
    ...
    <div class="filter-container">
      <el-button size="medium" type="primary" @click="onShowAdd">选定资产</el-button>
    </div>
    <el-table
    ...
    <SelAssetPkt v-if="addOrUpdateVisible" ref="selAssetPkt" @refreshDataList="onList" />
  </el-card>
</template>
...
import SelAssetPkt from './selAssetPkt.vue'

export default {
  name: 'List',
  components: { Pagination,SelAssetPkt},
  data() {
    return {
      search: {
        projectNameS: ''
  ...
  methods: {
    ...
    onShowAdd() {
      ...
      this.$nextTick(() => {
        this.$refs['selAssetPkt'].init(formObj, 'create')
      })
    },
    onShowUpdate(row) {
      ...
      this.addOrUpdateVisible = true
      this.$nextTick(() => {
        this.$refs['selAssetPkt'].init(formObj, 'update')
      })
    },
    ...
    onShowView(row){
      ...
      this.addOrUpdateVisible = true
      this.$nextTick(() => {
        this.$refs['selAssetPkt'].init(formObj, 'view')
      })
    }
  }
}
